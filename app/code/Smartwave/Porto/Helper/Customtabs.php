...
class Customtabs extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    $cms_tabs  = (string) ($this->getConfig('porto_settings/product/custom_cms_tabs') ?? '');
    $attr_tabs = (string) ($this->getConfig('porto_settings/product/custom_attr_tabs') ?? '');
    ...
    if ($cms_tabs !== '') {
        $tmp = @unserialize($cms_tabs);
        $cms_tabs_arr = is_array($tmp) ? $tmp : [];
    }

    if ($attr_tabs !== '') {
        $tmp = @unserialize($attr_tabs);
        $attr_tabs_arr = is_array($tmp) ? $tmp : [];
    }

    $parents = [];
    if (count($cms_tabs_arr) > 0 || count($attr_tabs_arr) > 0) {
        foreach ($product->getCategoryCollection() as $parent_cat) {
            $parents[] = (int)$parent_cat->getId();
        }
    }
    ...
    if (count($cms_tabs_arr) > 0) {
        foreach ($cms_tabs_arr as $_item) {
            if ($this->checkShowingTab($_item['category_ids'], $parents, $_item['product_skus'], $_sku)) {
                ...
                $custom_tabs[] = [
                    'tab_title'   => $_item['tab_title'] ?? '',
                    'tab_content' => $content,
                    'sort_order'  => (!empty($_item['sort_order']) && is_numeric($_item['sort_order'])) ? (int)$_item['sort_order'] : 0
                ];
            }
        }
    }

    if (count($attr_tabs_arr) > 0) {
        foreach ($attr_tabs_arr as $_item) {
            if ($this->checkShowingTab($_item['category_ids'], $parents, $_item['product_skus'], $_sku)) {
                $attr_code = $_item['attribute_code'] ?? '';
                if ($attr_code === '') continue;
